...
public class AimShootingMultiTouch : NetworkBehaviour
{
    ...
    */
    ...
    public void FireLaser(Vector3 mousePosition)
    {
        ...
        Ray2D ray = new Ray2D(playerPosition, direction);
        LayerMask mask = ~(1 << 11);
        RaycastHit2D hit = Physics2D.Raycast(playerPosition, direction, Mathf.Infinity, mask.value);
        ...
        while (continueBouncing)
        {
            if (hit.collider)
            {
                ...
                else if (hit.collider.tag.Equals("Mirror"))
                {
                    ...
                    ray = new Ray2D(hitPoint, reflected);
                    hit = Physics2D.Raycast(hitPoint + offsetReflection * normal, reflected, Mathf.Infinity, mask.value);
        ...
        laser.GetPositions(laserPositions);
        laser.enabled = true;
        //GUILog.Log(laserPositions.Length.ToString());
    }
    */
    ...
    [Command]
    void CmdUpdateTouchMap(Vector3 position, TouchPhase phase, int fingerId)
    {
        //GUILog.Log("recieved update touch map");
        RpcUpdateTouchMap(position, (int) phase, fingerId);
    }
    ...
    [ClientRpc]
    void RpcUpdateTouchMap(Vector3 position, int phase, int fingerId)
    {
        if (isLocalPlayer)
        {
            return;
        ...
        //GUILog.Log("recieved update touch map");
        UpdateTouchMap(position, (TouchPhase)phase, fingerId);
    }
}
